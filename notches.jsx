...
function drawNotches( paths, opts ){
    for( var sIdx = 0, sEnd = paths.length; sIdx < sEnd; sIdx++){
        ...
        for( var pIdx = 0, pEnd = pts.length; pIdx < pEnd; pIdx++){
            ...
            //log(["pIdx=[",pIdx,"],pNext:",pNext]);
            if( pNext < 0 ) break;
            
            if( isSegmentSelected(pts, pIdx)){
                cv = new Curve( path, pIdx, pNext );
                //log(["pIdx=[",pIdx,"],cv:",objectToString(cv)]);
                if( curves.length < 1 ){
                    curves.push([ cv ]);
                    
                } else {
                    var r = curves[ curves.length - 1 ];
					//log(["pIdx",pIdx,"curves.length-1",(curves.length-1),"r.length - 1",(r.length - 1),"r[ r.length - 1 ].idx2",r[ r.length - 1 ].idx2]);
                    if( r[ r.length - 1 ].idx2 == pIdx ){
                        ...
                        curves.push([ cv ]);
                        //log(["curves.push([ cv ])",objectToString(curves)]);
                    }
                }
            }
        }
        ...
        //log(["curves",objectToString(curves)]);
        ...
        var cv2 = curves[0];
        //alert(cv2.length);
        for(var i=0 ;i<cv2.length;i++){
			//alert("i = "+i);
			cv2[i].setParams();
			log("begin for")
			cv2[i].drawNotchesForT2(0,opts.notch_length, grp);
			//cv2[i].drawNotchesForT(0.5,opts.notch_length, grp);
			if(i == cv2.length-1){
				cv2[i].drawNotchesForT2(1,opts.notch_length, grp);
			}
		}
		//drawNotchesForT2
