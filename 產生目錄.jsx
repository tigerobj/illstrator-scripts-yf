...
// fmt ="%"+custName+"-前面-%"+"yyyyMMdd-nnn";

fmt ="yyyyMMdd-%"+custName+"_前面%-nnn";
extension = "eps";
folderPath = MyData["A4目錄輸出位置"];
dir = new Folder(MyData["A4目錄輸出位置"]);
if(!dir.exists){
  dir.create ();
}
last = findLastFileNameByCustName(fmt,folderPath,extension,custName+"_前面");
fileName = autoCode.execute(fmt,last);
openFileName = fileName;
//fmt = "yyMMdd-nnn";
copyAndOpenAIFile(MyData["A4網站底圖"],MyData["A4目錄輸出位置"],fileName+".eps");
destLayer = doc2.layers.getByName("服裝內容");
newItem2 = front.duplicate(destLayer);
newItem2.resize(25,25,true,true,true,true,25);
frameA = getPathItemByLayer("圖層 1","邊界");
frameB = getPathItemByThreeLevel("服裝內容","前面","前片","底色");
t = autoCenterXY(frameA,frameB);
newItem2.translate(t.left,t.top-20);
text = getTextFrameByThreeLevel("圖層 1","背景","變動文字","款示");
...
var doc = app.activeDocument;
fmt ="yyyyMMdd-%"+custName+"_後面%-nnn";
last = findLastFileNameByCustName(fmt,folderPath,extension,custName+"_後面");
fileName = autoCode.execute(fmt,last);
...
text = getTextFrameByThreeLevel("圖層 1","背景","變動文字","款示");
text.contents = custName;
removeItem = getGroupByLevel("圖層 1","QRCODE");
removeItem.remove();
try {
  var epsFile = doc2.fullName;
  var saveOpts  = new EPSSaveOptions();
...
function openEPS(fullPath) {
    var epsFile = new File(fullPath);
    if (epsFile.exists) {
        var doc = app.open(epsFile);
        return doc; // 回傳打開的文件
    } else {
        alert("檔案不存在: " + fullPath);
        return null;
    }
}


//folderPath,extension,custName+"_前面"
var fullEPSPath = MyData["A4目錄輸出位置"]+"/"+openFileName+".eps"; // 請改成你的實際路徑
var openedDoc = openEPS(fullEPSPath);
