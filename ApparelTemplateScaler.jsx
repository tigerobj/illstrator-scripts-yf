...
// 設定函數來移動ItemB到ItemA的頂部並距離指定的高度 (單位：mm)
// 使用Adobe Illustrator的ExtendScript，基於ECMA Edition 3語法編寫

function moveItemBToTopOfItemA(itemA, itemB, height) {
    // 確認兩個項目是否存在
    if (!itemA || !itemB) {
        alert("請確認傳入的項目是否有效。");
        return;
    ...
    var topA = boundsA[1]; // boundsA[1]是ItemA的頂部Y座標

    // 計算ItemB的新Y座標
    var newY = topA + height;

    // 獲取ItemB的邊界框並計算其目前的中心位置
    var boundsB = itemB.geometricBounds;
    var offsetY = newY - boundsB[1];

    // 移動ItemB到新位置
    itemB.translate(0, offsetY);
}

// 獲取ItemA和ItemB的高度
function getItemHeights(itemA, itemB) {
    // 確認兩個項目是否存在
    if (!itemA || !itemB) {
        alert("請確認傳入的項目是否有效。");
        return;
    ...
    // 計算高度
    var heightA = boundsA[1] - boundsA[3]; // 頂部Y座標減去底部Y座標
    var heightB = boundsB[1] - boundsB[3];

    return { heightA: heightA, heightB: heightB };
}

// 獲取ItemA的頂部到ItemB的頂部之間的高度
function getTopDistanceBetweenItems(itemA, itemB) {
    // 確認兩個項目是否存在
    if (!itemA || !itemB) {
        alert("請確認傳入的項目是否有效。");
        return;
    ...
    // 計算頂部之間的距離
    var distance = boundsA[1] - boundsB[1];

    return p2mm(distance);
}

// 獲取ItemA和ItemB的實際狀態值
function getItemActualValues(itemA, itemB) {
    // 確認兩個項目是否存在
    if (!itemA || !itemB) {
        alert("請確認傳入的項目是否有效。");
        return;
...
// 使用範例
// moveItemBToTopOfItemA(itemA, itemB, 320);
// var heights = getItemHeights(itemA, itemB);
// alert("ItemA的高度: " + heights.heightA + "，ItemB的高度: " + heights.heightB);
// var topDistance = getTopDistanceBetweenItems(itemA, itemB);
// alert("ItemA的頂部到ItemB的頂部之間的高度: " + topDistance);
// var actualValues = getItemActualValues(itemA, itemB);
// alert("ItemA的實際狀態: 左: " + actualValues.itemA.left + ", 頂: " + actualValues.itemA.top + ", 右: " + actualValues.itemA.right + ", 底: " + actualValues.itemA.bottom);
// alert("ItemB的實際狀態: 左: " + actualValues.itemB.left + ", 頂: " + actualValues.itemB.top + ", 右: " + actualValues.itemB.right + ", 底: " + actualValues.itemB.bottom);


//myObject = readCsvToObj(new File(csvfilePath));
configObject = getEnvCsvFile("configFilePath.csv");

//alert(configObject["工單路徑"]);
destinationPath = createFolderByFolderPath(configObject["工單路徑"]);
jpgPath = createFolderByPathForderName(destinationPath,"圖片輸出");
...
var selectedSizes = [];
showGui(pathEnvFile.fsName,selectedSizes,configObject);
var actualEntriesList = [];
clothingTemplateDoc = app.open(File(selectFilePathText));
//alert("selectedSizes.length : "+selectedSizes.length);



for (var sizeCnt = 0; sizeCnt < selectedSizes.length; sizeCnt++) {
	selectedValues = selectedSizes[sizeCnt];
	sourceCsvFilePath = getPath("data.csv");
	destinationCsvFilePath = destinationPath+"/data_"+selectedValues.selectedSize+".csv";
	copyCsvFile(sourceCsvFilePath,destinationCsvFilePath);
	selectedValues = selectedSizes[sizeCnt];
	var newEntries = [
	    ["dxf位置", selectedValues.dxfLocation],
	    ["前片", selectedValues.shirtFront],
	    ["後片", selectedValues.shirtBack],
	    ["袖片", selectedValues.shirtSleeve],
	    ["領片", selectedValues.shirtCollar],
	    ["底", selectedValues.background],
	    ["胸寬比例", selectedValues.ChestWidthRatio],
	    ["胸高比例", selectedValues.ChestHeightRatio]
	];
	//ChestWidthRatio  胸寬比例
	//ChestHeightRatio 胸高比例
	upsertMultipleToCsvFile(File(destinationCsvFilePath), newEntries);
...
/*	if(i == 2){
	...
	selectedValues = selectedSizes[sizeCnt];
	//alert("shirtType ： "+selectedValues.shirtType+" ,selectedSize : "+selectedValues.selectedSize);
	csvfilePath = getPath("data.csv");
	//alert("csvfilePath : "+csvfilePath);
	myObject = readCsvToObj(new File(csvfilePath));

	var mypath = getPath(myObject['尺寸']+".ai");
	dest = destinationPath+"/"+myObject['名稱']+"_"+myObject['尺寸']+".ai";
	var jpgFileName = jpgPath+"/"+myObject['名稱']+"_"+myObject['尺寸']+".jpg";
	//alert(destinationPath);
	copyAndOpenAiFile(mypath,dest);
	var doc = app.open(File(dest),DocumentColorSpace.CMYK);
	var layer = getLayerByName(doc, "設計層");
	if(!layer){
	...
	applyFillColorFromAnotherDoc(clothingTemplateDoc, "後內貼", doc, getMaskedPageItemByNameInLayer(doc, "前示意圖","後領遮罩", "後內貼"));

	//clothingTemplateDoc.close(SaveOptions.DONOTSAVECHANGES);
	...
	groupsCopyFromDoc(doc,clothingTemplateDoc,groupList);
	app.activeDocument = doc;

	var tmpGroup = doc.groupItems.add();
	//for (var i = 0; i < allPageItems.length; i++) {
	for(var pagei = 0; pagei < groupList.length; pagei++){
		if(getPageItemByName(groupList[pagei])){
			getPageItemByName(groupList[pagei]).move(tmpGroup, ElementPlacement.PLACEATEND);
		}
	}
	...
	myItem = getMaskedPageItemByNameInLayer(doc, "設計層","剪裁群組_底", "底");



	var myItemFillColor = myItem.fillColor;
	alignReplaceAndSetFill("前示意圖", "設計層","剪裁群組_底", "底");
	...
	myItem = getMaskedPageItemByNameInLayer(doc, "前示意圖","剪裁群組_底", "底");
	myItem.selected = true;
	myItem.fillColor=myItemFillColor;
	//movePageItemByHeight("左前","中心線",0,"底");
	// 將 "來源層" 中的 "物件A" 移動並對齊到 "目標層" 中的 "物件B" 的左上角
	//剪裁群組_底內的底是移動位置的參考物件跟右前的移動距離,真正移動的是剪裁群組_底

	var ItemB = getPageItemByNameInLayer(doc, "前示意圖", "剪裁群組_底");

	translatexy = alignPageItemToTopLeft2(myItem, ItemB, "前示意圖", "右前");
	//shirtFrontTeamName
	if(getPageItemByNameInLayer(doc, "前示意圖", "隊名")){
		getPageItemByNameInLayer(doc, "前示意圖", "隊名").translate(translatexy[0], translatexy[1]);
	}
	if(getPageItemByNameInLayer(doc, "前示意圖", "前數字")){
		getPageItemByNameInLayer(doc, "前示意圖", "前數字").translate(translatexy[0], translatexy[1]);
	}

	//var actualEntries = ;
	//設計層 隊名高度;230
	var hItemA = getPageItemByNameInLayer(doc, "前示意圖", "右袖");
	var hItemB = getPageItemByNameInLayer(doc, "前示意圖", "隊名");


	var actualEntries = [
	    ["隊名高度", getTopDistanceBetweenItems(hItemA, hItemB)]
	];
	...
	for(var ri = 0; ri < groupList.length; ri++){
		if(getPageItemByName(groupList[ri])){
			//alert(getPageItemByName(groupList[i]));
			getPageItemByName(groupList[ri]).move(tmpGroup, ElementPlacement.PLACEATEND);
		}
	}
	...
	myItem = getMaskedPageItemByNameInLayer(doc, "設計層","剪裁群組_後片", "後片");
	var myItemFillColor = myItem.fillColor;

	alignReplaceAndSetFill("後示意圖", "設計層","剪裁群組_後片", "後片");
	...
	myItem2.fillColor=myItemFillColor;
	//movePageItemByHeight("左前","中心線",0,"底");

	var ItemB = getPageItemByNameInLayer(doc, "後示意圖", "剪裁群組_後片");

	// 將 "來源層" 中的 "物件A" 移動並對齊到 "目標層" 中的 "物件B" 的左上角
	translatexy = alignPageItemToTopLeft2(myItem, ItemB, "後示意圖", "後片2");
	// 將 "來源層" 中的 "物件A" 移動並對齊到 "目標層" 中的 "物件B" 的左上角
...
//getPageItemByNameInLayer(doc, "前示意圖", "隊名").translate(translatexy[0], translatexy[1]);
	...
	getMaskedPageItemByNameInLayer(doc, "後示意圖","剪裁群組_後片", "後片").fillColor = 	myItem2.fillColor;
	myItem2.remove();
	...
	//得到後姓名的高度
	var hItemA = getPageItemByNameInLayer(doc, "後示意圖", "右後袖");
	var hItemB = getPageItemByNameInLayer(doc, "後示意圖", "姓名");


	actualEntries.push(["姓名高度", getTopDistanceBetweenItems(hItemA, hItemB)]);
	actualEntriesList.push(actualEntries);




	app.activeDocument = doc;
	saveAsJPEG(doc, jpgFileName);
	doc.save();
...
for(var sizeCnt = 0; sizeCnt < actualEntriesList.length; sizeCnt++) {
	selectedValues = selectedSizes[sizeCnt];
	if(selectedValues){
		destinationCsvFilePath = destinationPath+"/data_"+selectedValues.selectedSize+".csv";
		upsertMultipleToCsvFile(File(destinationCsvFilePath), actualEntriesList[sizeCnt]);
	}else{
		alert("sizeCnt:"+sizeCnt+" , error selectedSizes[sizeCnt] :"+selectedSizes[sizeCnt])
	}
}

clothingTemplateDoc.close(SaveOptions.SAVECHANGES);
//myObject = readCsvToObj(new File(csvfilePath));
