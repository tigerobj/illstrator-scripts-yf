...
for(var i=0;i<objs.length;i++){
	...
	if (group1Sizes.indexOf(clothesSize) !== -1) {
	    // alert(clothesSize + " 屬於 group1");
			...
			a = getGroupByTwoLevel("6扣棒球衣隊名處理-XL-縮放用","XL-後片", "姓名");
			//alert(a);
			if(a){
				ad = a.duplicate(targetLayer);
				ad.move(newGroupC, ElementPlacement.PLACEATEND);
			}

			a = getGroupByTwoLevel("6扣棒球衣隊名處理-XL-縮放用","XL-後片", "大背號");
			if(a){
				ad = a.duplicate(targetLayer);
				ad.move(newGroupC, ElementPlacement.PLACEATEND);
			}

			a = getGroupByTwoLevel("6扣棒球衣隊名處理-XL-縮放用","XL-後片", "後片logo");
			if(a){
				ad = a.duplicate(targetLayer);
				ad.move(newGroupC, ElementPlacement.PLACEATEND);
			}
	...
	if (group3Sizes.indexOf(clothesSize) !== -1) {
		...
		itemC.move(newGroupC, ElementPlacement.PLACEATEND);
		a = getGroupByTwoLevel("6扣棒球衣隊名處理-L-縮放用","L-後片", "姓名");
		if(a){
			ad = a.duplicate(targetLayer);
			ad.move(newGroupC, ElementPlacement.PLACEATEND);
		}

		a = getGroupByTwoLevel("6扣棒球衣隊名處理-L-縮放用","L-後片", "大背號");
		if(a){
			ad = a.duplicate(targetLayer);
			ad.move(newGroupC, ElementPlacement.PLACEATEND);
		}

		a = getGroupByTwoLevel("6扣棒球衣隊名處理-L-縮放用","L-後片", "後片logo");
		if(a){
			ad = a.duplicate(targetLayer);
			ad.move(newGroupC, ElementPlacement.PLACEATEND);
		}
