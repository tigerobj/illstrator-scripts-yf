...
function showGui() {
    ...
    // 👉 以後要加尺寸，只要改這個陣列
    // 客製化衣服配置檔.csv新增SA
    // 產生釣魚衣裁片圖SA	H:/杰優、裕豐工廠產品/創作/版型/杰優-R1126-釣魚服-S-5L-AI/裁片/SA_套版.eps
    // 套圖釣魚衣裁片圖SA	H:/杰優、裕豐工廠產品/創作/版型/杰優-R1126-釣魚服-S-5L-AI/套圖用/SA_套版.eps
    // SIZE_TABLE 要增加尺寸
...
function maskImageOnClipPiece(pieceName,cfg) {
    ...
    // forceCropImagesInGroup(newObj, pathItem);
    if("右袖" === cfg.label){
      // alert(newObj);
      // alert(pathItem);
    }
    forceImageCrop(pathItem, newObj);
    clipGrp = createClippingGroup(pathItem, newObj);


}

function forceImageCrop(pathItem, newObj){
  items = newObj.pageItems;
  for (var i = 0; i < items.length; i++) {
      // 檢查是否為點陣影像（RasterItem）或 連結影像（PlacedItem）
      if (items[i].typename == "RasterItem" || items[i].typename == "PlacedItem") {
          targetImg = items[i];
          var cropBox = pathItem.geometricBounds;
          ...
          // 重點在於第二個參數 cropBox
          app.activeDocument.rasterize(targetImg, cropBox, options);
          break; // 找到第一個就停止
      }
  }
}
