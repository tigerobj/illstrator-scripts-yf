...
function handleGlue(opt, paths){
    ...
    for(i = 0; i < lines.length; i++){
        ...
        if(isSelected(p[0])){
        	//alert("p[0] x="+p2mm(p[0].anchor[0])+",y = "+p2mm(p[0].anchor[1]));
            processed = true;
        ...
        if(isSelected(p[p.length - 1])){
        	//alert("p["+(p.length - 1)+"] x="+p2mm(p[p.length - 1].anchor[0])+",y = "+p2mm(p[p.length - 1].anchor[1]));
            processed = true;
...
function handleGlue1(p, paths, right_direction, conf, opt){
    ...
    //alert(objectToString(paths));
//    alert(paths[0].name);
//    alert(paths[1].name);
    //copyText("p:"+objectToString(p));
    ...
    if(mp.d < 0){
        ...
        //copyText("np_spec xxx ==== "+objectToString(np_spec));
...
function showPP(pp){
	...
	for(var i = 0; i < pp.length; i++){
		str = str+"x["+i+"]="+p2mm(pp[i].anchor[0])+"mm ,y["+i+"]="+p2mm(pp[i].anchor[1])+"mm\n\n";
		
	}
	alert(str);
}

function anchorTomm(p){
	var str = "["+p2mm(p[0])+"mm ,";
	str = str+p2mm(p[1])+"mm]";
	return str;
}
...
function roughMeasureing(p, paths, conf){
    ...
    var measureing = function (b, p){
        result.d = -1;
        var str = "p : "+anchorTomm(p)+"\n";
        str = str+"b.q[0] : "+listPointTomm(b.q[0])+"\n";
        str = str+"b.q[1] : "+listPointTomm(b.q[1])+"\n";
        str = str+"b.q[2] : "+listPointTomm(b.q[2])+"\n";
        str = str+"b.q[3] : "+listPointTomm(b.q[3])+"\n";
        str = str+"b.x : "+listPointTomm(b.x)+"\n";
        str = str+"b.y : "+listPointTomm(b.y)+"\n";
        
        for(var t = 0; t <= 1; t += conf.INITIAL_T_STEP){
            var d = b.dist2(t, p);
            str = str+"t = "+t+" , d = "+d ;
            str = str+" ,b.pnt(t) = "+anchorTomm(b.pnt(t))+"\n";
            if(result.d < 0 || d < result.d){
                result.d = d;
                result.t = t;
            }
        }
        str = str +"result.d = "+result.d+" ,result.t = "+result.t;
        //copyText(str);
    };

    //pp[idx1].rightDirection
    for(var i = 0; i < paths.length; i++){
        var pp = paths[i].pathPoints;
        //alert(paths[i].name+" ,pp["+i+"].length = "+pp.length);
        //showPP(pp);
        var found = false;
        ...
        for(var pp_idx = 0; pp_idx < pp.length; pp_idx++){
            ...
            //pp[pp_idx].anchor = pp[nidx].anchor]
            // aborts processing if both ends are same point
            
            //alert("pp_idx="+pp_idx+" ,nidx="+nidx+"], b.a0 = ["+anchorTomm(b.a0)+"], b.a1 = ["+anchorTomm(b.a1)+"]");
            
            if( arrEq(b.a0, b.a1) ) continue;
                
            measureing(b, p.anchor);
            alert("i = "+i+",pp_idx = "+pp_idx);
...
var Bezier = function(pp, idx1, idx2){
  ...
  //alert("pp["+idx1+"].rightDirection = "+anchorTomm(pp[idx1].rightDirection)+"\n pp["+idx2+"].leftDirection = "+anchorTomm(pp[idx2].leftDirection));
  //alert("x list = "+listPointTomm(this.x));
  //alert("y list = "+listPointTomm(this.y));
}
// --------------------------------------
Bezier.prototype = {
  pnt : function(t){
...
//this.q = [pp[idx1].anchor, pp[idx1].rightDirection,
//          pp[idx2].leftDirection, pp[idx2].anchor];
//n=0 [-x0+3(x0_R-x3_L)+x3,3(x0-2x0_R+x3_L),3(x0_R-x0),x0]
